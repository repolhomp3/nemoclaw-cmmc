...
|---|---|---|---|---|---|---|---|---|---|---|
| POAM-001 | Finalize cryptographic implementation statement | Critical | Decision + Evidence | SC.L2-3.13.8, SC.L2-3.13.11, SC.L2-3.13.16, IA.L2-3.5.10 | Current crypto document is still a draft worksheet and not a final host-backed claim | Identify exact deployed crypto modules/versions, confirm approved/FIPS basis, collect host evidence, approve final statement. Use `docs/evidence/poam-001-crypto-host-evidence-checklist.md` against the target DGX Spark host. | [Security Architecture] | [TBD] | Module/version evidence, TLS/SSH configs, storage encryption proof, approved architecture decision | In Progress |
...
| POAM-019 | Baseline and restrict Docker administrative access | Critical | Configuration + Access Control | AC family, CM family, SI family | Docker is in scope but admin access and daemon configuration are not yet fully evidenced or locked down in the readiness package | Identify all Docker-admin-capable users/groups, restrict to approved admins only, record daemon configuration baseline, and collect evidence of review | [System Administrator / Security] | [TBD] | Group membership evidence, daemon config, approval record, baseline package update | Open |
| POAM-020 | Produce Docker runtime proof package for assessed build | High | Evidence + Test | CM family, SC family, SI family, assessor readiness | The repo references container controls generally, but lacks one coherent runtime proof package for the actual Docker deployment posture on the DGX Spark | Capture container inventory, image digests, published ports, bind mounts, effective privilege posture, and host listen exposure from the target DGX Spark | [System Administrator / Assessment Lead] | [TBD] | Docker inventory evidence pack, image digest list, published-port proof, local-only exposure validation | Open |
| POAM-021 | Define approved container image allowlist and intake evidence path | High | Procedure + Evidence | CM family, MA family, MP family, RA family | Image intake is covered generally, but the assessed build does not yet show a bounded list of approved production images with traceable intake evidence | Create a production image allowlist by digest/tag, map each to intake/scanning records, and tie the set into baseline configuration evidence | [Security / Operations] | [TBD] | Approved image list, scan records, intake records, baseline package references | Open |
